...
public class ArffLoader {
    ...
    protected void setValue(Instance instance, int numAttribute, double value, boolean isNumber) {
        ...
        if (numAttribute<=this.instanceInformation.classIndex())
        {
        	...
        	// set instance id, based on the data from the column after class
        	if (numAttribute==this.instanceInformation.classIndex()+1)
        	{
        		instance.setInstanceId(value);
        	}else
        		// set instance time stamp, based on the data from the column after instance id
        		if (numAttribute==this.instanceInformation.classIndex()+2)
        		{
        			instance.setInstanceTimeStamp(value);
        		}
